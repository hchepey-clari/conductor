 ...
 * MetricRegistry is ingested to the Conductor's Spectator metric library to gather the metrics and
 * org.coursera:metrics-datadog is used to periodically upload metrics. Metric Name format used by
 * MetricRegistry is not compatible with datadog reporter for resolving tags by default.
 *
 * <p>Reference: @see <a
 * href="https://github.com/coursera/metrics-datadog#tag-encoding-and-expansion">Coursera Datadog
 * Reporter tags</a>
 */
public class ConductorMetricsNameFormatter implements MetricNameFormatter {
    ...
    private final BiPredicate<String, String>
            metricsTagFilter; // Predicate which filters tags for the given metric
    private final MetricTagValueFormatter metricTagValueFormatter;

    ConductorMetricsNameFormatter(
            BiPredicate<String, String> metricsTagFilter,
            MetricTagValueFormatter metricTagValueFormatter) {
        ...
        try {
            ...
            for (int idx = 1; idx < segments.length; idx++) {
                ...
                if (metricsTagFilter.test(metricName, tagName)) {
                    ...
                    String formattedTagVal =
                            metricTagValueFormatter.getFormattedTagValue(
                                    metricName, tagName, tagVal);
            ...
            LOGGER.error(
                    "Exception formatting the metric name {} with message {}",
                    name,
                    ex.getMessage());
